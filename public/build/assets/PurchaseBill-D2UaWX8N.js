...
import { n as PaymentPanel, r as TransactionSearchBar, t as ConfirmClearDialog } from "./ClearConfirmDialog-CxMk0XiX.js";
...
import { a as DialogTitle, i as DialogHeader, n as DialogContent, t as Dialog } from "./dialog-EmwkokJQ.js";
import { Dt as router3, Ot as usePage, St as ChevronDown, Tt as toast, gt as Plus, lt as Trash2, mt as Search, ot as X, rt as formatCurrency, xt as ChevronRight } from "./app-DQEL3DJY.js";
import { n as calculateSellingUnitCost, t as calculateMargin } from "./product-adapter-Df3GNTgA.js";
import { n as MobileCartList, t as MobilePaymentDrawer } from "./MobilePaymentDrawer-WcLhMXIZ.js";
import { t as ConfirmTransactionDialog } from "./ConfirmDialog-CDo0w4Oy.js";
import { t as purchaseStrategy } from "./purchase-C4O3n5X1.js";
//#region resources/js/Pages/purchases/PurchaseBill.tsx
var import_react = /* @__PURE__ */ __toESM(require_react(), 1);
var import_jsx_runtime = require_jsx_runtime();
function SupplierCombobox({ supplier, onSelect, onClear }) {
	const [open, setOpen] = (0, import_react.useState)(false);
	const [query, setQuery] = (0, import_react.useState)("");
	const contacts = (usePage().props.suppliers || []).map((s) => ({
		...s,
		currentBalance: s.current_balance ?? s.opening_balance ?? 0
	}));
	const suppliers = (0, import_react.useMemo)(() => contacts, [contacts]);
	const filtered = (0, import_react.useMemo)(() => {
		if (!query) return suppliers;
		const q = query.toLowerCase();
		return suppliers.filter((s) => s.name.toLowerCase().includes(q) || (s.phone || "").includes(q) || (s.contactPerson || "").toLowerCase().includes(q));
	}, [query, suppliers]);
...
function PurchaseBillPage() {
	...
	const mockProducts = (inertiaProps.products || []).map((p) => ({
		...p,
		id: p.id,
		name: p.name,
		sku: p.sku,
	...
	const filteredProducts = (0, import_react.useMemo)(() => {
		...
		return mockProducts.filter((p) => (p.name || "").toLowerCase().includes(q) || (p.sku || "").toLowerCase().includes(q) || ((p.category?.name || p.category || "") + "").toLowerCase().includes(q)).slice(0, 8);
	}, [search]);
	...
	const handleChangeUnit = (0, import_react.useCallback)((itemId, unitId) => {
		...
		if (unitId === product.baseUnitId) {
			...
			if (!opt) return;
			const costPerBase = product.purchaseConfig ? product.purchaseConfig.cost / product.purchaseConfig.quantity : item.unitCost / item.purchasePackQty;
			const newCost = Math.round(costPerBase * opt.factor * 1e4) / 1e4;
			setCart((prev) => prev.map((c) => c.id !== itemId ? c : {
				...c,
	...
	const addToCart = (0, import_react.useCallback)((product) => {
		const raw = mockProducts.find((p) => String(p.id) === String(product.id));
		if (!raw) return;
		...
		const baseUnitDef = getUnit(raw.baseUnitId || "piece") || {
			id: "piece",
			name: "Piece",
												...
												children: [
													...
													/* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
														...
														onBlur: (e) => {
															...
															if (!isNaN(v) && v > 0 && v !== item.purchaseQuantity) updateQuantity(item.id, v - item.purchaseQuantity);
															else if (isNaN(v) || v <= 0) target.value = String(item.purchaseQuantity);
														},
														onKeyDown: (e) => {
													...
													/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
														className: "text-[10px] text-muted-foreground w-8 text-left",
														children: item.purchasePackName
													}),
													/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
														onClick: () => updateQuantity(item.id, 1),
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(MobileCartList, {
						items: cart.map((c) => {
							const prod = mockProducts.find((p) => p.id === c.productId);
							const customOpts = prod ? purchaseStrategy.getCustomUnitOptions({
								id: prod.id,
								name: prod.name,
								sku: prod.sku,
							...
							return {
								...
								sellingUnits: [{
									id: "purchase-pack",
									name: c.purchasePackName
								}, {
									id: c.baseUnitId,
									name: "Per " + c.baseUnitName
								}],
								customUnits: customOpts.length > 0 ? customOpts.map((o) => ({
									id: o.id,
									label: o.label
								})) : void 0,
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
									...
									onClick: () => {
										...
										router3.visit(`/purchases/${receiptId}`);
									},
