...
import { Dt as router3, Ot as usePage, St as ChevronDown, Tt as toast, _t as Package, a as getCurrentUserName, mt as Search, ot as X, rt as formatCurrency } from "./app-DQEL3DJY.js";
...
function ReturnPage({ strategy, backPath, title, isPurchase }) {
	...
	const rawTransactions = inertiaProps.transactions || [];
	const transactions = (0, import_react.useMemo)(() => {
		return (isPurchase ? rawTransactions.map((b) => ({
			id: b.id,
			ref: b.invoice_ref,
			date: b.date,
			party: b.supplier?.name || b.supplier_name,
			total: b.total_amount,
			items: (b.items || []).length,
			itemsList: b.items || []
		})) : rawTransactions.map((s) => ({
			id: s.id,
			ref: s.invoice_number,
			date: s.date,
			party: s.customer?.name || s.customer_name || "Walk-in Customer",
			total: s.grand_total,
	...
	const selectTransaction = (0, import_react.useCallback)((tx) => {
		const raw = rawTransactions.find((r) => r.id === tx.id);
		if (!raw) return;
		setOriginalTx(raw);
		setReturnItems((raw.items || []).map((item) => ({
