...
export default function CustomerLedgerPage() {
  ...
  if (!contact) {
    return (
      <ReportLayout title="Customer Ledger" subtitle="Select a customer to view their financial statement" icon={<Users className="size-5 text-primary" />}>
        <div className="text-center py-24 text-sm text-muted-foreground">
          <Users className="size-12 mx-auto mb-3 text-muted-foreground/20" />
          <p>No customer selected. Navigate from a customer profile or use the contact filter.</p>
        </div>
      </ReportLayout>
    )
  }
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="rounded-xl border border-border bg-card p-3.5">
          ...
          <div className="text-lg font-bold text-foreground mt-1 tabular-nums">{formatCurrency(openingBalance)}</div>
        </div>
        <div className="rounded-xl border border-border bg-card p-3.5">
        ...
        <div className="rounded-xl border border-border bg-card p-3.5">
          ...
          <div className={cn('text-lg font-bold mt-1 tabular-nums', closingBalance > 0 ? 'text-red-500' : 'text-emerald-600')}>
            ...
            {closingBalance > 0 ? ' (Receivable)' : closingBalance < 0 ? ' (Credit)' : ''}
          </div>
        </div>
      </div>
