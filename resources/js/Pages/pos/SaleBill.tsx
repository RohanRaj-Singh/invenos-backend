...
export default function SaleBillPage() {
  ...
  // Normalize backend snake_case → camelCase for prototype compatibility
  ...
  const posCustomers = (inertiaProps as any).customers || []

  function filterPOSProducts(query: string): any[] {
  ...
  const sessionCount = useMemo(() => posProducts.length + 1, [posProducts])
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <TransactionSearchBar
        ...
        onAddProduct={(product) => {
          const fullProduct = posProducts.find((p: any) => String(p.id) === String(product.id))
          if (fullProduct) addToCart(fullProduct as any)
        }}
